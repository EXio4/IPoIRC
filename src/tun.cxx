...
void TunModule::worker_reader(Tun& tun, Comm::Socket socket) {
    ...
    while ((nbytes = zmq_recv(socket, sbuffer, MTU, 0)) >= 0) {
        ...
        if (nbytes == 0) {
            ...
            loc_log(Log::Warning) << "some message got truncated by " << nbytes - MTU << "(" << nbytes << " - " << MTU << "), this means the MTU is too low for you!" << std::endl;
        }
...
void TunModule::worker_writer(Tun& tun, Comm::Socket socket) {
    ...
    while ((nbytes = tun.read(sbuffer, MTU)) != 0) {
        if (nbytes > 0) {
            loc_log(Log::Debug) << "got " << nbytes << " from tun" << std::endl;
            if (zmq_send(socket, sbuffer, nbytes, 0) < 0) {
                loc_log(Log::Warning) << "error when trying to send a message to the irc thread:" << zmq_strerror(errno) << std::endl;
            }
        } else {
